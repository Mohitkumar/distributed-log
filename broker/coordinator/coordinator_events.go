...
func (c *Coordinator) ApplyNodeAddEvent(nodeID, addr, rpcAddr string) error {
	...
	eventData, err := protocol.EncodeAddNodeEvent(protocol.AddNodeEvent{NodeID: nodeID, Addr: addr, RpcAddr: rpcAddr})
	if err != nil {
		return err
	}
	data, err := protocol.EncodeMetadataEvent(&protocol.MetadataEvent{
...
func (c *Coordinator) ApplyNodeRemoveEvent(nodeID string) error {
	...
	eventData, err := protocol.EncodeRemoveNodeEvent(protocol.RemoveNodeEvent{NodeID: nodeID})
	if err != nil {
		return err
	}
	data, err := protocol.EncodeMetadataEvent(&protocol.MetadataEvent{
